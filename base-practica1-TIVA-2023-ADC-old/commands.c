...
static int Cmd_reboot(int argc, char *argv[])
{
    ...
    // Print some header text.
    //
    UARTprintf("system will be reboot");
    SysCtlReset();
    // Return success.
    return(0);
}
...
tCmdLineEntry g_psCmdTable[] =
{
    ...
    { "reboot",     Cmd_reboot,      "     : software reboot" },
#if ( configUSE_TRACE_FACILITY == 1 )
...
static void vCommandTask( void *pvParameters )
{
	...
	UARTprintf("> ");    
    uint8_t valor = 0x0F;
    ACME_setPinDir(valor);
    ACME_writePin(valor);
    uint8_t rega = 0xFF , regb = 0x00;
    ACME_setIntType(rega,regb);
...
int32_t I2CDriver_Open()
{
   ...
   // I2CLoopbackEnable(I2C1_BASE);// he conectado secsor Hamed
