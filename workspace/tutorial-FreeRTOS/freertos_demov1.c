//*****************************************************************************
//
// freertos_demo.c - Simple FreeRTOS example.v1
//
...
static portTASK_FUNCTION(LEDTask,pvParameters)
{
    ...
    // Print demo introduction (opcional) --> Nunca usar en el "main" --> posible error si cadena muy larga
    //
    ...
    while(1)
    {
    	//SysCtlDelay( (SysCtlClockGet()/(3*1000))*ui32Intervalo) ;	//Espera ineficiente (bucle)
        vTaskDelay(ui32Intervalo);
...
int main(void)
{
    ...
    pui32Parametros1[0]=0.5 * configTICK_RATE_HZ;  // ms
...
//#if 0
    if((xTaskCreate(LEDTask, "Led1", LED1TASKSTACKSIZE, pui32Parametros1,tskIDLE_PRIORITY + LED1TASKPRIO, NULL) != pdPASS))
        {
            while(1)
            { // Aqui me quedo si ha habido un error al crear la tarea
            }
        }
//#endif
...
#if 0
    pui32Parametros2[0]=0.5 * configTICK_RATE_HZ; // ms
...
#if 1
    pui32Parametros2[0]=0.6 * configTICK_RATE_HZ; // ms
    pui32Parametros2[1]=GPIO_PIN_2;
    if((xTaskCreate(LEDTask, "Led2", LED2TASKSTACKSIZE, pui32Parametros2,tskIDLE_PRIORITY + LED2TASKPRIO, NULL) != pdPASS))
        {
            while(1)
