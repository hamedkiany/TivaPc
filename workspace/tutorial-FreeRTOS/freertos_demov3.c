//*****************************************************************************
//
// freertos_demo.c - Simple FreeRTOS example.v3
//
...
#define LED1TASKSTACKSIZE 128    // Tamaño de pila para la tarea LED1TASK --> cambiar a 200
#define LED2TASKPRIO 1           // Prioridad para la tarea LED2TASK
#define LED2TASKSTACKSIZE 128    // Tamaño de pila para la tarea LED2TASK  --> cambiar a 200
...
static portTASK_FUNCTION(LEDTask,pvParameters)
{
    ...
    volatile uint8_t pui8Gran_array[475]; // Cambio para revisar STACK_OVERFLOW

    while(1)
    {
       uint32_t i;
       for (i=0;i<475;i++) pui8Gran_array[i]=0xFF; // Cambio para revisar STACK_OVERFLOW
...
//#if 0
    if((xTaskCreate(LEDTask, "Led1", 512, pui32Parametros1,tskIDLE_PRIORITY + LED1TASKPRIO, NULL) != pdPASS))
        {
            while(1)
            {
            }
...
//#if 0
    ...
    if((xTaskCreate(LEDTask, "Led2", 512, pui32Parametros2,tskIDLE_PRIORITY + LED2TASKPRIO, NULL) != pdPASS))
        {
            while(1)
            {
            }
