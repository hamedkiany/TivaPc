//*****************************************************************************
//
// freertos_demo.c - Simple FreeRTOS example.v8
//
...
#include "semphr.h"              // FreeRTOS: definiciones relacionadas con tareas
...
static portTASK_FUNCTION(LEDTask,pvParameters)
{
    ...
    while(1)
    {
        ...
        xSemaphoreTake(g_pUARTSemaphore,portMAX_DELAY);
        UARTprintf("\n Esta es la tarea %s hablando por el puerto serie cada %d \n",pcTaskGetTaskName(NULL),ui32Intervalo);
        xSemaphoreGive(g_pUARTSemaphore);
    }
}
...
int main(void)
{
    ...
    pui32Parametros1[0]= 0.011 * configTICK_RATE_HZ;  // 0.011 segundos
...
//#if 0
    pui32Parametros2[0]= 0.013 * configTICK_RATE_HZ;  // 0.013 segundos
