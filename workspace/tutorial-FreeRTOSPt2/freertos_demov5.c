//*****************************************************************************
//
// freertos_demo.c - Simple FreeRTOS example.v5
//
...
void GPIOFIntHandler(void){
	...
	{
		xSemaphoreGiveFromISR(semaforo_freertos1,&higherPriorityTaskWoken);//Abre el semaforo FreeRTOS
		pui32Parametros1[0]=1;
	}
		else
		    pui32Parametros1[0]=0;

    if (!(i32PinStatus & RIGHT_BUTTON))
    {
        xSemaphoreGiveFromISR(semaforo_freertos1,&higherPriorityTaskWoken);//Abre el semaforo FreeRTOS
        pui32Parametros1[1]=2;
    }
        else
            pui32Parametros1[1]=0;
