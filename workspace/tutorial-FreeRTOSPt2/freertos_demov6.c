//*****************************************************************************
//
// freertos_demo.c - Simple FreeRTOS example.v6
//
...
#define SW1TASKPRIO 2            // Prioridad para la tarea SW1TASK
#define SW1TASKSTACKSIZE 128     // Tamaño de pila para la tarea SW1TASK
#define SW2TASKPRIO 1            // Prioridad para la tarea SW2TASK
#define SW2TASKSTACKSIZE 128     // Tamaño de pila para la tarea SW2TASK
...
{
	xSemaphoreTake(semaforo_freertos1,0);	//Nos aseguramos de que el semaforo esta cerrado
    ...
    while(1)
    {
        xSemaphoreTake(semaforo_freertos1,portMAX_DELAY); //Semaforo del RTOS
...
//*****************************************************************************
//
// This task waits for a data at the queue, decodes the pushed buttons and
// flash the corresponding LED
//
//*****************************************************************************

//static void Switch2Task(void *pvParameters)
static portTASK_FUNCTION(Switch2Task,pvParameters)
{
    ...
    while(1)
    {
    	if (xQueueReceive(cola_freertos,&ui32Status,portMAX_DELAY)==pdTRUE){ // Cambiar portMAX_DELAY por 500 para apagado automatico 5 seg despues
    																	     // de que la cola esta vacia
           // Decodifica lo recogido en ui32Status y enciende los LEDs correspondientes
    		if(ui32Status == 0){
                MAP_GPIOPinWrite(GPIO_PORTF_BASE, GPIO_PIN_1|GPIO_PIN_2|GPIO_PIN_3,GPIO_PIN_1);
    		}else if (!(ui32Status & LEFT_BUTTON)){
                MAP_GPIOPinWrite(GPIO_PORTF_BASE, GPIO_PIN_1|GPIO_PIN_2|GPIO_PIN_3,GPIO_PIN_2);
    		}else if (!(ui32Status & RIGHT_BUTTON)){
                MAP_GPIOPinWrite(GPIO_PORTF_BASE, GPIO_PIN_1|GPIO_PIN_2|GPIO_PIN_3,GPIO_PIN_3);
    		}
    		vTaskDelay(1000); // Espera 1000 ticks encendido
            MAP_GPIOPinWrite(GPIO_PORTF_BASE, GPIO_PIN_1|GPIO_PIN_2|GPIO_PIN_3,0); // Comentar/eliminar estas dos si se coloca una espera
            vTaskDelay(500);  // Espera 500 ticks apagado                      // distinta de portMAX_DELAY
        }else{  //Por esta rama nunca llega excepto que la espera no sea portMAX_DELAY
                //Valor de Status no se ha leido
...
//#endif
    ...
    // Crea la cola de mensajes con espacio para 16 elementos tipo uint32_t
    cola_freertos=xQueueCreate(16,sizeof(uint32_t));
    if (NULL==cola_freertos)
    	while(1);            // Si hay problemas para crear la cola, se queda aqui.
