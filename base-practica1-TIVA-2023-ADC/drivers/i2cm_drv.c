...
//#include "driverlib/sysctl.h" //by Hamed
//#include "driverlib/gpio.h"//by Hamed
//#include "driverlib/pin_map.h" //by Hamed
...
//*****************************************************************************
static void
I2CMStateIdle(tI2CMInstance *psInst, tI2CMCommand *pCommand,portBASE_TYPE *higherPriorityTaskWoken)
{
    //
...
//        return;
//    }
    if (xQueueReceiveFromISR(psInst->I2CcomandsQueue,pCommand,higherPriorityTaskWoken) == pdFALSE)
    {
        return;
    }
                         //
...
I2CMIntHandler(tI2CMInstance *psInst)
{
    ...
    //
    pCommand = &psInst->OnGoingI2CTransacion; //(psInst->pCommands[psInst->ui8ReadPtr]);
    ...
    while(1)
    {
        ...
        switch(psInst->ui8State)
        {
            ...
            case STATE_IDLE:
            {
                ...
                //
                I2CMStateIdle(psInst, pCommand,&higherPriorityTaskWoken);
            ...
            case STATE_CALLBACK:
            {
                ...
                //sustir por que coresponde HAMED actualizar datos de OnGoing
                //
                // This state is done and the next state needs to be handled
                // immediately.
                //
...
//    psInst->ui8WritePtr = 0;
    ...
    psInst->I2CcomandsQueue=xQueueCreate(NUM_I2CM_COMMANDS,sizeof(tI2CMCommand));
    if (psInst->I2CcomandsQueue == 0)
    {
        while(1);
    }
    //
    ...
    //
    MAP_IntPrioritySet(ui8Int,configMAX_SYSCALL_INTERRUPT_PRIORITY);
...
//    pCommand = &(psInst->pCommands[psInst->ui8WritePtr]);
    ...
    //
    Command.ui8Addr = ui8Addr;
    Command.pui8WriteData = pui8WriteData;
    Command.ui16WriteCount = ui16WriteCount;
    Command.ui16WriteBatchSize = ui16WriteBatchSize;
    Command.pui8ReadData = pui8ReadData;
    Command.ui16ReadCount = ui16ReadCount;
    Command.ui16ReadBatchSize = ui16ReadBatchSize;
    Command.pfnCallback = pfnCallback;
    Command.pvCallbackData = pvCallbackData;

    //
    // Update the write pointer.
    //
    //psInst->ui8WritePtr = ui8Next;
    xQueueSend(psInst->I2CcomandsQueue,&Command,portMAX_DELAY);
    //
...
I2CMTransferResume(tI2CMInstance *psInst, uint8_t *pui8Data)
{
    ...
    if(psInst->ui8State == STATE_WRITE_PAUSE)
    {
        psInst->OnGoingI2CTransacion.pui8WriteData = pui8Data;
    }
    else
    {
        psInst->OnGoingI2CTransacion.pui8ReadData = pui8Data;
    }
