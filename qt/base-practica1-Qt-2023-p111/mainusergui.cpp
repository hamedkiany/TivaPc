  ...
  , transactionCount(0)
{
    ...
    connect(ui->checkBox1,SIGNAL(toggled(bool)),this,SLOT(cambiaLEDs()));
    connect(ui->checkBox2,SIGNAL(toggled(bool)),this,SLOT(cambiaLEDs()));
    connect(ui->checkBox3,SIGNAL(toggled(bool)),this,SLOT(cambiaLEDs()));
...
void MainUserGUI::on_Knob_valueChanged(double value)
{
    ...
    double value_a = ui->Knob->value();
    if (value_a > 5)
    {
        parameter.leds.red=0;
        parameter.leds.green=0;
        parameter.leds.blue=1;

    }
    else
       // ui->ledB->setChecked(false);
        parameter.leds.blue=0;
